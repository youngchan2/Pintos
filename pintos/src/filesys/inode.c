...
static bool inode_increase(struct inode_disk *inode_disk, off_t start, off_t end, uint32_t is_dir, bool init)
{
  ...
  if (init)
  {
    ...
    {
      inode_disk->direct_map_table[i] = -1;
    }
    inode_disk->indirect_block_sec = -1;
    inode_disk->double_indirect_block_sec = -1;
  }
...
bool inode_create(block_sector_t sector, off_t length, uint32_t is_dir)
{
  ...
  if (disk_inode != NULL)
  {
    if (inode_increase(disk_inode, 0, length, is_dir, true))
    {
                     ...
                     off_t offset)
{
  ...
  {
    inode_increase(&inode_disk, inode_disk.length, size + offset, 0, false);
